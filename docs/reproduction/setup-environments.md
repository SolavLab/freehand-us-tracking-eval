# Environments for regenerating raw recordings

You don't need any of this to use `vipose` — see [`docs/setup.md`](../setup.md).
This page is for the two audiences who do: someone regenerating this
repository's own pose CSVs from the raw sensor recordings, and a reviewer
checking how thoroughly the manuscript's numbers were reproduced.

## Reproduction ladder

| Rung | What it regenerates | Environment | Time |
|---|---|---|---|
| **R1** | Every table and figure, from the per-frame residuals that ship in `datasets/` | the `vipose` install | seconds |
| **R2** | The full evaluation — synchronization, hand–eye calibration, residuals — from the pose and mocap CSVs that ship | the `vipose` install | ~4 min |
...
planned for a separate Zenodo deposit) and vendor SDKs, and is not
...
re-runnable, not so it can be replayed to identical output; see
`extract/README.md` for what was actually measured when it was re-run.

---

## `zed-sdk` — pipeline I pose extraction (R3a)

Python **3.12** with the Stereolabs ZED SDK and its Python bindings.
...
> The raw recordings behind the shipped example dataset (not themselves in
> this repository — see `raw.yaml`, and the reproduction ladder above) were
> made on a Jetson Orin NX with **ZED SDK 5.0.7** under JetPack 6.2.1. That is
> the acquisition-side version and is independent of the version used here
> for offline pose estimation.

---

## `ros-convert` — RealSense bag conversion (R3b)

Python **3.10**, ROS 2 **Humble**, plus `pyrealsense2`.

`rosbag2_py`, `rclpy` and `cv_bridge` come from the ROS 2 installation, not
from pip. Only `pyrealsense2` and `numpy` come from the virtual environment.

**Source order matters.** ROS 2 must be sourced *before* the virtual
...
regardless of whether the virtual environment includes system site-packages.
Done the other way round, the ROS imports fail.
...
made the raw recordings behind the shipped example dataset.

---

## `isaac-ros` — cuVSLAM pipelines (R3c)

A Docker container built on NVIDIA's Isaac ROS Development Environment,
modified to carry both the ZED and the RealSense stacks. Nothing runs on the
host.
...
`environments/isaac_ros/README.md`. In summary: add the Isaac ROS apt
repository to the RealSense image layer; pin and install ZED SDK 5.1 for
cu12/Ubuntu 22 with the ROS ZED message packages; add a ZED layer and its
entrypoint; and add an entrypoint-additions layer.
